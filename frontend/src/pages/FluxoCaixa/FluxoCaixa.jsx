...
export default function FluxoCaixa() {
  ...
  const mesesDisponiveis = [
    ...new Set(
      todasMovimentacoes.filter((m) => m.data).map((m) => m.data.slice(0, 7))
    ),
  ].sort((a, b) => (a < b ? 1 : -1));
  ...
  // Ordena do mais antigo, acumula saldo corrente, depois reverte para exibir o mais recente primeiro
  let saldoCorrido = 0;
      ...
      <div className="fluxo-container">
        ...
        <div className="fluxo-cards">
          ...
          <div className="fluxo-card saldo">
            <h2>Saldo Final</h2>
            <span>{formatarMoeda(totalEntradas - totalSaidas)}</span>
          </div>
        </div>
        ...
        <div className="movimentacoes-box">
          ...
          <table>
            ...
            <tbody>
              {paraExibir.length === 0 ? (
                <tr>
                  <td colSpan="5" className="fluxo-vazio">Nenhuma movimentação encontrada.</td>
                </tr>
              ) : (
                paraExibir.map((m) => (
                  <tr key={m.id}>
