...
export default function ResumoSemanal() {
  ...
  entradas.forEach((m) => {
    ...
    if (!produtosMap[chave]) {
      produtosMap[chave] = { quantidade: 0, unidade: m.unidade || "" };
    }
    produtosMap[chave].quantidade += Number(m.quantidade) || 0;
  });
  const produtoMaisVendido = Object.entries(produtosMap).sort(
  ...
  const receitaPorDia = {};
  entradas.forEach((m) => {
    if (!receitaPorDia[m.data]) receitaPorDia[m.data] = 0;
    receitaPorDia[m.data] += Number(m.valor);
  });
  const melhorDiaEntry = Object.entries(receitaPorDia).sort(
    (a, b) => b[1] - a[1]
  )[0];
  const melhorDia = melhorDiaEntry
    ? {
        nome: DIAS_SEMANA[new Date(melhorDiaEntry[0] + "T00:00:00").getDay()],
        valor: melhorDiaEntry[1],
      }
    : null;
  ...
  saidas.forEach((m) => {
    ...
    if (!gastosMap[cat]) gastosMap[cat] = 0;
    gastosMap[cat] += Number(m.valor);
  });
  const maiorGasto = Object.entries(gastosMap).sort(
    (a, b) => b[1] - a[1]
  )[0];

  const semMovimentacoes = movimentacoesSemana.length === 0;
      ...
      <div className="semanal-container">
        ...
        <div className="semanal-cards">
          ...
          <div className="semanal-card saldo">
            <h2>Lucro Semanal</h2>
            <span>R$ {lucroSemanal.toFixed(2)}</span>
          </div>
        </div>

        {semMovimentacoes ? (
          <p className="sem-dados">
            Nenhuma movimentação registrada nessa semana.
          </p>
        ) : (
          <div className="semanal-infos">
            <div className="info-box">
              ...
              {produtoMaisVendido ? (
                <>
                  ...
                  <p>
                    {produtoMaisVendido[1].quantidade}{" "}
                    {produtoMaisVendido[1].unidade} vendidos na semana
                  </p>
                </>
              ) : (
            ...
            <div className="info-box">
              ...
              {melhorDia ? (
                <>
                  <span>R$ {melhorDia.valor.toFixed(2)}</span>
                  <p>{melhorDia.nome}</p>
                </>
              ) : (
                <span className="sem-info">—</span>
